...
public class ConsoleControl {
    ...
    public void insert (ModelProduct product){
        ...
        try{
            ...
            state.executeUpdate("INSERT INTO `consolas`(`name`, `number`, `price`, `type`, `discount`) VALUES ('"+product.getName()+"',"+product.getNumber()+","+product.getPrice()+",'"+product.getType()+"',"+product.getDiscount()+")");
    ...
    public void delete(int id)
    {
        try {
            state = ConnectionDB.openConnection().createStatement();
            state.executeUpdate("Delete FROM `consolas` WHERE id = " + id);
            ConnectionDB.closeConnection();
        } catch (SQLException ex) {
            ConnectionDB.closeConnection();
            Logger.getLogger(ProductControl.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void update(ModelProduct product)
    {
        try {
            state = ConnectionDB.openConnection().createStatement();
            state.executeUpdate("UPDATE consolas set `name` = "+"'"+product.getName()+"'"+", `number` = "+product.getNumber() +", `price` = " + product.getPrice() +", `type` = "+"'"+product.getType()+"'"+", `discount` = "+product.getDiscount()+" WHERE id = "+product.getId() );
            ConnectionDB.closeConnection();
        } catch (SQLException ex) {
            Logger.getLogger(ProductControl.class.getName()).log(Level.SEVERE, null, ex);
            ConnectionDB.closeConnection();
    ...
    public ObservableList<ModelProduct>getAllProduct(){
        ...
        try {
            state = ConnectionDB.openConnection().createStatement();
            ResultSet result = state.executeQuery("SELECT * FROM consolas");
    ...
    public ObservableList<ModelProduct>getSearchProduct(String name){
        ...
        try {
            state = ConnectionDB.openConnection().createStatement();
            ResultSet result = state.executeQuery("SELECT * FROM consolas WHERE name LIKE '%"+name+"%'");
